...
static bool wait_for(volatile uint8_t *pin_reg, uint8_t mask, uint8_t level, uint32_t timeout_us)
{
...
static uint32_t measure_pulse(volatile uint8_t *pin_reg, uint8_t mask, uint8_t level, uint32_t timeout_us)
{
    uint32_t start = micros();
    //while (digitalRead(pin) == level)
    while (((*pin_reg & mask) ? 1 : 0) == level)
...
static bool read_sensor(float *temp, float *hum)
{
    ...
    if (!wait_for(&DHT22_PINR, (1 << DHT22_BIT), LOW,  100))
        return false; // wait for sensor to pull LOW
    if (!wait_for(&DHT22_PINR, (1 << DHT22_BIT), HIGH, 150))
        return false; // wait for LOW to end (~80 µs)
    if (!wait_for(&DHT22_PINR, (1 << DHT22_BIT), LOW,  150))
        return false; // wait for HIGH to end (~80 µs)

    // Read 40 bits. Each bit: 50 µs LOW start pulse, then HIGH whose duration
    // encodes the value (< 50 µs = 0, > 50 µs = 1).
    for (uint8_t i = 0; i < 40; i++)
    {
        if (!wait_for(&DHT22_PINR, (1 << DHT22_BIT), HIGH, 100)) return false; // wait for 50 µs LOW to end
        uint32_t high_us = measure_pulse(&DHT22_PINR, (1 << DHT22_BIT), HIGH, 150);
        if (high_us == 0) 
            return false;
